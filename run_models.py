...
def create_model_progressive(xtrain, input_shape=500):
    inputs = Input(shape=input_shape)
    scaler = Normalization()
    scaler.adapt(xtrain)
    scaled_inputs = scaler(inputs)
    x = Dense(100, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(scaled_inputs)
    x = Dense(100, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(x)
    x = Dropout(0.2)(x)
    x = Dense(100, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(x)
    x = Dropout(0.2)(x)
    x = Dense(100, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(x)
    x = Dropout(0.2)(x)
    x = Dense(100, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(x)
    x = Dense(50, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(x)
    output = Dense(1, activation='sigmoid')(x)
    model = Model(inputs=inputs, outputs=output)
    return model


def create_model_ann(xtrain, input_shape=500):
    inputs = Input(shape=input_shape)
    scaler = Normalization()
    scaler.adapt(xtrain)
    scaled_inputs = scaler(inputs)
    x = Dense(200, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.0005))(scaled_inputs)
    x = Dense(100, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.0005))(x)
    x = Dropout(0.2)(x)
    x = Dense(100, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.0005))(x)
    x = Dropout(0.2)(x)
    x = Dense(100, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.0005))(x)
...
for label in labels:
    ...
    for model_name in model_names:
        ...
        # chosen = 0
        for k, chosen in enumerate(dates):
            if k < 6:
                continue
            if k == len(dates) - 1:
                break
        # for train_index, test_index in kf.split(X):
            ...
            for synthetic_percent in synthetic_percent_list:
                ...
                # # hold out each day as test date
                # test_idx = y[(y.date == chosen) & (y.type == 0)].index
                # train_idx = y[(y.date != chosen) & (y.type == 0)].index
                
                # training progressively
                test_idx = y[(y.date.isin(dates[k+1:])) & (y.type == 0)].index
                train_idx = y[(y.date.isin(dates[:k+1])) & (y.type == 0)].index                
                ...
                if synthetic_percent < ratio:
                    synthetic_percent = np.round(ratio + 0.1, 1)
                ...
                model = create_model_progressive(X_train, input_shape=X_train.shape[1])
                ...
                history = model.fit(X_train, y_train, batch_size=256, epochs=200, validation_data=(X_test, y_test),
                                    callbacks=[EarlyStopping(patience=10, min_delta=0.0005, restore_best_weights=True)])
                ...
                if not os.path.isfile(basedir + f'/../model_performance_{model_name}.csv'):
                    pd.DataFrame(model_performance, columns=['label', 'model_name', 'test_date', 'synthetic_%', 'train_accuracy', 'train_recall', 'train_precision', 'test_accuracy', 'test_recall', 'test_precision', 'test_accuracy_avg', 'test_recall_avg', 'test_precision_avg']).to_csv(basedir + f'/../model_performance_{model_name}.csv', index=False)
                else:
                    pd.DataFrame(np.array(model_performance[-1]).reshape(1, -1), columns=['label', 'model_name', 'test_date', 'synthetic_%', 'train_accuracy', 'train_recall', 'train_precision', 'test_accuracy', 'test_recall', 'test_precision', 'test_accuracy_avg', 'test_recall_avg', 'test_precision_avg']).to_csv(basedir + f'/../model_performance_{model_name}.csv', mode='a', index=False, header=False)                  
    ...
    for model_name in model_names:
        ...
        # for train_index, test_index in kf.split(X):
            ...
            for synthetic_percent in synthetic_percent_list:
                ...
                tf.keras.backend.clear_session() # release resource associated with previous model
                model = create_model_ann(X_train, input_shape=X_train.shape[1])
                print('Normalization layer adapted: ', model.layers[1].is_adapted)
